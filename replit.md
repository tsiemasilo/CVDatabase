...
## Recent Updates
- November 20, 2025: **EDIT BUTTON NAVIGATION FIX**: Fixed critical edit button functionality for all user roles - implemented proper SPA navigation using setActiveTab and history.pushState instead of hard page reloads, added form state cleanup to prevent stale data when navigating away from edit mode, implemented popstate listener to synchronize UI with browser back/forward navigation, preserved query parameters during edit navigation, added aggressive cleanup of form/edit state on unmount and when exiting edit mode. Edit button now correctly routes users with capture permissions to Capture record page with record loaded for editing, while users without capture permissions continue using inline edit modal. Architect-approved implementation eliminates stale data issues and ensures UI stays aligned with URL during browser navigation
- October 22, 2025: **PDF BLANK PAGES FIX**: Resolved critical PDF generation issue creating 4 pages instead of 2 with blank pages 2 and 4 - identified root cause as conflicting CSS classes (page-break-before + no-page-break) and forced height constraints (minHeight: 297mm) on Page 2 container, removed contradictory page break directives, eliminated rigid height/flex constraints (flex-col, flex-grow), simplified Page 2 layout to flow naturally after Page 1's page-break-after footer, PDF now generates exactly 2 clean pages without overflow or blank canvases
